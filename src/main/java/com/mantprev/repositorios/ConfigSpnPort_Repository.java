...
@Repository
public interface ConfigSpnPort_Repository extends CrudRepository <ConfigSpn_Port, Integer>{ 

	
	
	@Query("select C.estatusOTs from ConfigSpn_Port C where C.estatusOTs <> '' ")
    List<String> getItemsDeStatusDeOTs();
	
	@Query("select C from ConfigSpn_Port C")
    List<ConfigSpn_Port> getConfigSpinners();
