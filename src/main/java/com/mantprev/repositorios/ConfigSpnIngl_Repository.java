...
@Repository
public interface ConfigSpnIngl_Repository extends CrudRepository <ConfigSpn_Ingl, Integer>{

	
	
	@Query("select C.estatusOTs from ConfigSpn_Ingl C where C.estatusOTs <> '' ")
    List<String> getItemsDeStatusDeOTs();
	
	@Query("select C from ConfigSpn_Ingl C")
    List<ConfigSpn_Ingl> getConfigSpinners();
